...
| Still unproved | reduced-event conversion lemma and corresponding AN19 asymptotic runtime | Blocked | finite counters and tests do not establish an asymptotic class bound |
...
For the parametric Figure 6 membership events generated by
`An19WeightedPetal` after source power-of-two length rounding, prove an
explicit upper bound on the number of equivalence classes of the exact reduced
directed costs

`ell(u,v) + d(x,u) - d(x,v)`,

over the relevant radii and highway intervals, and justify the ordering
transformation used by the reduced-event implementation with enough detail to
derive AN19's claimed `O(m log n log log n)` runtime. The formal SIAM text does
not state this conversion. Observed event-class counts, workspace-scan bounds,
or finite differential tests cannot substitute for the lemma.
...
`8f9ab06ce00c1e80a58e5b6302c14a408fefabd7`. The SIAM PDF title page was
...
| paper-table generator, run twice | 0 | `paper-tables.md` and `scope-table.csv` hashes are stable |
...
| `cargo test --workspace` | 0 | 247 passed, 3 existing ignored, 13 suites, 408.12 s |
...
| `cargo test --workspace` | 0 | 410.44 s | 247 passed and 3 existing release-scale campaigns ignored across 13 suites |
| `RUSTDOCFLAGS="-D warnings" cargo doc --workspace --no-deps` | 0 | 4.17 s | 7 package documentation sets generated without warnings |
| `cargo build --workspace --release` | 0 | 16.79 s | 6 crates compiled successfully |
| `python3 tools/check_release_consistency.py` | 0 | 2.71 s | 10 runs, 499220 grid comparisons, 174767 polygon rows/components, and 27228 CP-SAT components verified |
...
| `git diff af46e74..HEAD -- results` | 0 | <0.01 s | no stale or regenerated release evidence changed |
