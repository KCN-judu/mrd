...
| AN19 static low-stretch hierarchy | `rect-graph::source_an19` | `source_an19::hierarchy::Lsst::construct`, `source_an19::event::engine::Exact`, `source_an19::event::engine::Reduced`, `source_an19::event::engine::Proved` | exact hierarchy plus an isolated exact all-radii fixed-snapshot engine, complete semantic/queue traces, six empirical charge maps, a local event-cardinality certificate, and a stable exact binary heap with a practical comparison certificate | AN19 states `O(m log n log log n)` | tree/stretch/certificate checks plus exact Oracle/reduced differential, A--H fixtures, local/practical-bound mutation rejection, and ten trace-mutation classes | `ExactStaticLsstOracle` and `source_an19::event::engine::Exact` on bounded graphs | P9.3.2d hard blocker: fixed-snapshot semantics, `O(n+m)` event/item cardinality, and an `O((n+m) log(n+m))` practical heap comparison bound are proved, but the checked SIAM source does not prove the source-equivalent `O(m+n log log n)` queue bound or global amortization, so tests do not verify the asymptotic runtime |
