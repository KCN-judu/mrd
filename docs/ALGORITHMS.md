...
| AN19 static low-stretch hierarchy | `rect-graph::source_an19` | `An19HierarchicalLsst::construct` | exact hierarchy with source-rounded lengths, certified counters, and complete workspace-scan reconstruction | AN19 states `O(m log n log log n)` | exact tree/stretch/certificate checks, 32 focused tests, mutation and scaling regressions | `ExactStaticLsstOracle` on bounded graphs | P9.3.2d hard blocker: the checked SIAM source does not prove the reduced-event ordering/counting conversion, so tests do not verify the asymptotic runtime |
