...
| Geometric completion, SG Section 10 | `rect-oracle-sg` | `complete_with_backend` | indexed component-local frontier by default in CompactOnly; reference rescan retained | linear completion after chord choice in the source construction | exact selected/added cut and canonical-rectangle differential populations | `ReferenceRescanCompletion` | ordinary grid regions only |
