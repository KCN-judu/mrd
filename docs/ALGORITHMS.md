...
| AN19 static low-stretch hierarchy | `rect-graph::source_an19` | `An19HierarchicalLsst::construct`, `ExactEventOracle`, `An19ReducedEventEngine`, `ProvedEventEngine` | exact hierarchy plus an isolated exact all-radii fixed-snapshot engine, complete semantic/queue traces, and six empirical charge maps | AN19 states `O(m log n log log n)` | tree/stretch/certificate checks plus exact Oracle/reduced differential, A--H fixtures, and ten trace-mutation classes | `ExactStaticLsstOracle` and `ExactEventOracle` on bounded graphs | P9.3.2d hard blocker: fixed-snapshot semantics are implemented, but the checked SIAM source does not prove the reduced-event bound, global amortization, or priority-queue bound, so tests do not verify the asymptotic runtime |
