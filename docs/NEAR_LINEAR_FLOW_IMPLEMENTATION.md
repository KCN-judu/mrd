...
| Definitions 5.6--5.8, Theorem 5.1 | d-level tree chain, shifted single branch, compact cycle representation, hidden-stability data structure | `graph::source_min_ratio::{model,chain,cycle,query,execution}` | P9.4 provides immutable source-tree snapshots, stable branch IDs, deterministic shifted selection, compact-cycle decoding, and finite ledger transitions. P9.5a remains blocked: no source construction maps a live `CertifiedIpmSnapshot` and anonymous stable-ledger coordinates to a selected compact cycle plus its current update certificate. P8's `dynamic_min_ratio::{oracle,experiment}` remains the permanent enumerating/replay baseline and cannot be imported by production. No Theorem 5.1 query or bound claim is made. |
