...
| Theorem 8.4, P9.3.4b | Certified finite-domain witness expander | `graph::source_spanner::experiment::{complete,domain}` | deterministic complete-graph witness only for at most 20 nodes when every exact input weight satisfies `w <= n-1 <= 18w`; exhaustive nontrivial-cut expansion certificate and full recomputation; all other inputs reject explicitly, so this is not CGLNPS20's general construction or a runtime claim |
| Theorem 8.5, P9.3.4c | Certified finite-domain one-level expander decomposition | `graph::source_spanner::experiment::{decomposition,domain}` | accepts a connected graph of at most 20 nodes only when its full edge set forms one level that passes capacity, exact minimum-degree floor, and exhaustive cut-expansion checks; records the explicit sole component and every source edge exactly once, then recomputes the complete certificate. Disconnected, multi-level, and out-of-domain cases reject. This is not the general deterministic decomposition or a source runtime claim. |
...
| Theorem 8.1, P9.3.4e | Certified finite Algorithm 4 replay | `graph::source_spanner::algorithm4::{witness,first_embedding,finalize}` | source Algorithm 4 (arXiv:2309.16629v1, pp. 41--42) is replayed for the currently certified single-level, single-component finite domain: witness union with exact `phi 2^i` weight, `W -> J` bounded path/threshold/deletion traces, direct finite `J -> W` paths, image subgraph, and exact existing composition audit. Multi-level components, Theorem 8.6's general pruning, and Theorem 8.1 bounds remain unimplemented and unclaimed. |
| Definitions 5.6--5.8, Theorem 5.1 | d-level tree chain, shifted single branch, compact cycle representation, hidden-stability data structure | `graph::source_min_ratio::{input,model,chain,cycle,candidate,terminal,query,execution}` | P9.4 provides immutable source-tree snapshots, stable branch IDs, deterministic shifted selection, compact-cycle decoding, and finite ledger transitions. P9.5a has exact IPM/source/circulation provenance through `input::Input`, an exact heap over explicitly declared fundamental spanner/tree candidates through `candidate::Registry`, and a source-shaped single-snapshot terminal tree that emits its non-tree fundamental declarations through `terminal::Tree`. `source_flow::iteration::Step::from_terminal_candidate` now bridges only that terminal heap to an exact direction after requiring exact equality with the terminal input coordinates. Core/spanner embedding provenance, rejected-core declarations, cross-snapshot candidate maintenance, and complete-candidate differential evidence remain absent. P8's `dynamic_min_ratio::{oracle,experiment}` remains the permanent enumerating/replay baseline and cannot be imported by production. No Theorem 5.1 selection or bound claim is made. |
