...
| Definitions 5.6--5.8, Theorem 5.1 | d-level tree chain, shifted single branch, compact cycle representation, hidden-stability data structure | `graph::source_min_ratio::{input,model,chain,cycle,candidate,terminal,query,execution}` | P9.4 provides immutable source-tree snapshots, stable branch IDs, deterministic shifted selection, compact-cycle decoding, and finite ledger transitions. P9.5a has exact IPM/source/circulation provenance through `input::Input`, an exact heap over explicitly declared fundamental spanner/tree candidates through `candidate::Registry`, and a source-shaped single-snapshot terminal tree that emits its non-tree fundamental declarations through `terminal::Tree`. It still lacks core/spanner embedding provenance, cross-snapshot candidate maintenance, and the current update certificate needed to select a compact cycle for `Step`. P8's `dynamic_min_ratio::{oracle,experiment}` remains the permanent enumerating/replay baseline and cannot be imported by production. No Theorem 5.1 selection or bound claim is made. |
