...
def scope_rows() -> list[dict[str, str]]:
    return [
        ...
        {"feature": "effective chord enumeration", "theoretical paper": "O(n log n)", "current Rust artifact": "grid::experiment::InteriorRuns for grids; chord::experiment::Sweep for accepted ordinary polygons", "tested": "three-backend exact family differential", "notes": "ordinary-loop sweep is O(n log n + q); formal-boundary source cases remain unsupported"},
        ...
        {"feature": "polygon structural validation", "theoretical paper": "ordinary rectilinear domain", "current Rust artifact": "polygon::experiment::Validator with polygon::oracle::Validator", "tested": "accepted and negative-category differential", "notes": "deterministic integer event ordering"},
        ...
        {"feature": "workspace scan-count audit", "theoretical paper": "implementation accounting prerequisite", "current Rust artifact": "Complete", "tested": "247 passed; 3 existing ignored", "notes": "counters and invariants are internally consistent at 8f9ab06; this is not an asymptotic proof"},
        {"feature": "formal SIAM AN19 source identification", "theoretical paper": "Abraham--Neiman, SIAM J. Comput. 48(2), DOI 10.1137/17M1115575", "current Rust artifact": "Complete", "tested": "source checked", "notes": "the reduced-event conversion was not found in the cited source"},
        {"feature": "AN19 logarithmic reduced-class conversion", "theoretical paper": "not established by the cited source", "current Rust artifact": "Refuted", "tested": "parameterized regression plus algebraic proof", "notes": "a power-of-two chord family has O(log N) original classes and Omega(N) exact reduced-cost classes"},
        {"feature": "AN19 exact reduced-event semantics", "theoretical paper": "fixed-projection Figure 6 event contract", "current Rust artifact": "Complete on tested domain", "tested": "31 Oracle/reduced A--H snapshots", "notes": "exact rational trace and six empirical charge maps; no runtime proof"},
        {"feature": "AN19 fixed-snapshot event cardinality", "theoretical paper": "local all-radii event work", "current Rust artifact": "Proved", "tested": "machine certificate in every Oracle/reduced run", "notes": "semantic events <= 3n + 4m + 2 and queue items <= n + 2m + 2; comparison time excluded"},
        {"feature": "AN19 exact reduced-event ordering replacement", "theoretical paper": "required for the local P9.3.2d runtime chain", "current Rust artifact": "Implemented semantics / proof deferred", "tested": "bounded exact differential only", "notes": "the future proved backend must bound de-potentialized exact rational thresholds without one priority class per reduced cost"},
        {"feature": "AN19 runtime verification", "theoretical paper": "O(m log n log log n)", "current Rust artifact": "Pending low-priority proof", "tested": "no", "notes": "workspace scans do not close the missing mathematical reduction"},
        {"feature": "P9.3.2d", "theoretical paper": "AN19 static LSST prerequisite for Lemma 5.4", "current Rust artifact": "Faithful implementation complete", "tested": "exact Oracle differential and trace audit", "notes": "proof debt is deferred to low-priority P9.6a after P9.5; P9.3.3 through P9.5 may proceed, but no AN19 runtime claim is permitted"},
        {"feature": "almost-linear theoretical flow backend", "theoretical paper": "used asymptotically", "current Rust artifact": "P9.5 implementation pending", "tested": "no", "notes": "the source-shaped semantic chain may proceed; the AlmostLinear claim awaits low-priority P9.6a proof closure"},
