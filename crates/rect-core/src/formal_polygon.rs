...
use crate::{
    Boundary, Coord, DoubledPoint, HorizontalChord, HorizontalChordId, OrthogonalEdgeIndex,
    OrthogonalLoop, Point, PolygonError, PolygonLoopId, RectilinearDomain, RectilinearPolygon,
...
impl FormalRectilinearPolygon {
    ...
    /// Constructs every effective chord using Section 10 Step 1(a)--(d).
    ///
    /// # Errors
    ///
    /// Returns a structured validation or generated-chord error.
    pub fn effective_chords_source(
        &self,
    ) -> Result<FormalChordConstructionResult, FormalPolygonError> {
        let incidence = self.incidence()?;
        let geometry = build_vertex_geometry(self, &incidence);
        construct_effective_chords(self, &incidence, &geometry)
    }
...
mod tests {
    ...
    fn source_construction_matches_figure_three_and_exercises_merge_delete_steps() {
        ...
        let oracle = polygon.effective_chords_pairwise().unwrap();
        let source = polygon.effective_chords_source().unwrap();
