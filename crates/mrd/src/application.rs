...
use dominance::experiment::{
    ChordEnumerator, Mode, PathTreeOrientationPolicy, PolygonArrangementBackend,
    PolygonChordBackend, PolygonCompletionBackend, PolygonSolveOptions, RegionBackend,
    Representation, Verification, complete_formal_polygon, solve_polygon_with_options,
    solve_with_representation_and_region_dual_and_orientation_policy,
};
use graph::source_an19::event::campaign::{Campaign, Family};
use mrd_domain::{
...
use sg_oracle::grid::CompletionBackendKind;
use sg_oracle::{polygon_cut_index, polygon_sparse};
...
enum Command {
    ...
    Benchmark {
        ...
        families: String,
        /// Versioned request for one paper-scaling sample. The external runner
        /// supplies this relative JSON path to keep each measurement process
...
struct FormalSolveOutput {
    ...
    effective_chords: mrd_domain::FormalFamilies,
    step_two_transformation: dominance::experiment::FormalStep2Transformation,
    explicit_matching: graph::Matching,
    explicit_vertex_cover: graph::VertexCover,
    compact_vertex_cover: graph::VertexCover,
    selected_horizontal: Vec<bool>,
    selected_vertical: Vec<bool>,
    completion: sg_oracle::grid::PolygonCompletionResult,
}

#[derive(Deserialize, Serialize)]
struct PreservedExperimentManifest {
    schema_version: usize,
    runs: Vec<verification::benchmark::BenchmarkMetadata>,
    #[serde(
        default,
...
fn run() -> Result<(), CliError> {
    ...
    match cli.command {
        ...
        Command::Polyomino {
            ...
            let summary = verification::polyomino::verify_polyominoes(max_cells, oracle_cell_limit);
...
fn exhaustive_command(width: usize, height: usize, output: Option<&Path>) -> Result<(), CliError> {
    match verification::grid::exhaustive_binary(width, height) {
        Ok(report) => {
...
fn random_command(
    ...
    match verification::grid::random_binary(width, height, cases, seed) {
        Ok(report) => {
...
fn export_adversarial(output_dir: &Path) -> Result<(), CliError> {
    ...
    let instances = verification::adversarial::endpoint_contact_instances()
        .into_iter()
        .chain(verification::adversarial::topological_stress_instances())
        .chain(verification::adversarial::external_oracle_adversarial_instances())
        .chain([
            verification::adversarial::dense_conflict_grid(4, 5),
            verification::adversarial::dense_conflict_grid(8, 8),
        ])
        .collect::<Vec<_>>();
...
fn search_path_tree_witness_command(
    ...
    let report = verification::witness::search_path_tree_witnesses(
        max_width,
        max_height,
        seed,
        require_clean,
...
fn benchmark_command(options: BenchmarkOptions<'_>) -> Result<(), CliError> {
    ...
    let context = benchmark_context()?;
    if suite == BenchmarkSuiteArg::PaperScaling {
        let request_path = paper_scaling_request.ok_or_else(|| {
            CliError::Input(
                "paper-scaling requires --paper-scaling-request <relative-json-request>".to_owned(),
            )
        })?;
        let request = serde_json::from_slice::<verification::paper_scaling::Request>(&fs::read(
            request_path,
        )?)?;
    ...
    if suite == BenchmarkSuiteArg::PolygonBackendDifferential {
        let report = verification::polygon_campaign::extended_polygon_backend_campaign(
            context,
            max_cells,
            random_cases,
            sizes,
    ...
    if suite == BenchmarkSuiteArg::PolygonNegative {
        let report = verification::polygon_campaign::polygon_negative_campaign(context);
        write_json(&report, Some(output))?;
        update_manifest(
            &output.with_file_name("manifest.json"),
            report.metadata.clone(),
    ...
    if suite == BenchmarkSuiteArg::PolygonNativeFixtures {
        let report =
            verification::polygon_campaign::native_polygon_fixture_campaign(context, sizes);
        write_json(&report, Some(output))?;
        write_json(
            &report.minimized_counterexamples,
            Some(&output.with_extension("counterexamples.json")),
    ...
    if suite == BenchmarkSuiteArg::PolygonScaling {
        let report = verification::polygon_campaign::polygon_scaling_campaign(context, sizes);
    ...
    if suite == BenchmarkSuiteArg::FormalFixtures {
        let report = verification::formal_campaign::formal_fixture_campaign(context);
        write_json(&report, Some(output))?;
        update_manifest(
            &output.with_file_name("manifest.json"),
            report.metadata.clone(),
    ...
    if suite == BenchmarkSuiteArg::CleanCensus {
        let census = verification::benchmark::clean_census_4x4(context);
    ...
    if suite == BenchmarkSuiteArg::CleanBoundaryDifferential {
        let report = verification::benchmark::clean_boundary_differential_4x4(context);
        write_text(output, &report.to_csv())?;
        write_json(&report, Some(&output.with_extension("json")))?;
    ...
    if suite == BenchmarkSuiteArg::PathTreeGapDifferential {
        ...
        let report = verification::gap_differential::verify_gap_backends(
            context,
            verification::gap_differential::GapDifferentialConfig {
                polyomino_max_cells: max_cells,
                random_cases,
                complete_bipartite_max_t: maximum_t,
                family_scales: sizes.to_vec(),
                ..verification::gap_differential::GapDifferentialConfig::default()
            },
        );
        write_text(output, &report.to_csv())?;
        write_json(&report, Some(&output.with_extension("json")))?;
    ...
    let report = match suite {
        BenchmarkSuiteArg::Adversarial => verification::benchmark::benchmark_adversarial(context),
        BenchmarkSuiteArg::Construction => {
            let report = verification::benchmark::benchmark_biclique_construction(context, sizes);
            write_text(output, &report.to_csv())?;
            write_json(&report, Some(&output.with_extension("json")))?;
            let manifest_path = output
                .parent()
        ...
        BenchmarkSuiteArg::CleanCompleteBipartite => {
            verification::benchmark::benchmark_clean_complete_bipartite(context, sizes)
        }
        BenchmarkSuiteArg::CleanCompleteBipartiteCompact => {
            verification::benchmark::benchmark_clean_complete_bipartite_compact(context, sizes)
        }
        BenchmarkSuiteArg::PathTreeComparison => {
            verification::benchmark::benchmark_path_tree_comparison(context, sizes)
        }
        BenchmarkSuiteArg::PathTreeFamilies => {
            verification::benchmark::benchmark_path_tree_geometry_families(
                context,
                sizes.iter().copied().max().unwrap_or(5),
            )
        }
        BenchmarkSuiteArg::PathTreeScaling => {
            verification::benchmark::benchmark_path_tree_geometry_scaling(context, sizes)
        }
        BenchmarkSuiteArg::PathTreeVs4d => {
            let report = verification::benchmark::benchmark_path_tree_vs_4d(context, sizes);
            write_text(output, &report.to_csv())?;
            write_json(&report, Some(&output.with_extension("json")))?;
            let manifest_path = output
                .parent()
        ...
        BenchmarkSuiteArg::PathTreeAdvantage => {
            let report =
                verification::benchmark::benchmark_path_tree_advantage(&context, sizes, 16);
            write_text(output, &report.to_csv())?;
            write_json(&report, Some(&output.with_extension("json")))?;
            write_text(&output.with_extension("md"), &report.to_markdown())?;
            let manifest_path = output
        ...
        BenchmarkSuiteArg::PathTreeOrientationAudit => {
            let report =
                verification::benchmark::benchmark_path_tree_orientation_audit(context, sizes);
            write_text(output, &report.to_csv())?;
            write_json(
                &serde_json::json!({
        ...
        BenchmarkSuiteArg::PathTreeDualDifferential => {
            let report =
                verification::benchmark::benchmark_path_tree_dual_differential(context, sizes);
            write_text(output, &report.to_csv())?;
            write_json(
                &serde_json::json!({
                    "metadata": report.metadata.clone(),
        ...
        BenchmarkSuiteArg::AutoFallback => {
            verification::benchmark::benchmark_auto_fallback(context)
        }
        BenchmarkSuiteArg::DenseConflict => {
            verification::benchmark::benchmark_dense_conflict(context, sizes)
        }
        BenchmarkSuiteArg::DenseCompactOnly => {
            verification::benchmark::benchmark_dense_compact_only(context, sizes)
        }
        BenchmarkSuiteArg::DenseCompletion => {
            verification::benchmark::benchmark_dense_completion(context, sizes)
        }
        BenchmarkSuiteArg::CompletionHeavy => {
            verification::benchmark::benchmark_completion_heavy(context, sizes, families)
        }
        BenchmarkSuiteArg::AreaHeavy => {
            verification::benchmark::benchmark_area_heavy(context, sizes)
        }
        BenchmarkSuiteArg::Polyomino => {
            verification::benchmark::benchmark_polyomino(context, max_cells, oracle_cell_limit)
        }
    ...
    for fixture in &report.failure_fixtures {
        verification::minimize::write_regression_bundle(
            Path::new("test-data/regressions"),
            fixture,
        )
        .map_err(|error| CliError::Output(error.to_string()))?;
    }
...
fn compare_external_command(
    ...
    let external: verification::external::ExternalOracleResult =
        serde_json::from_slice(&external_bytes)?;
    let report = verification::external::compare_external(
        &grid,
        &input_hash,
        &external,
        exact_cover_cell_limit,
...
fn generate_command(
    ...
    let instance = match family {
        GenerateFamilyArg::CleanCompleteBipartite => {
            verification::adversarial::clean_complete_bipartite_grid(t.ok_or_else(|| {
                CliError::Input("clean-complete-bipartite requires --t".to_owned())
            })?)
            .map_err(|error| CliError::Input(error.to_string()))?
        }
        GenerateFamilyArg::DenseConflict => {
            verification::adversarial::dense_conflict_grid(horizontal, vertical)
        }
    };
    ...
    let geometry = sg_oracle::grid::analyze_geometry_with(
        component,
            ...
            let result = solve_polygon_with_options(
                ...
                PolygonSolveOptions {
                    verification_mode: match solver {
                        SolverArg::DominanceCompressed => Verification::FullyAudited,
                        SolverArg::DominanceCompactOnly => Verification::CompactOnly,
        ...
        LoadedInput::FormalPolygon(polygon) => {
            ...
            if let Some(svg_path) = svg {
                let rendered = PolygonDissectionResult {
                    ...
                    diagnostics: mrd_domain::Diagnostics::default(),
                    certificate: None,
                };
                write_text(
                    svg_path,
...
fn solve_component<C>(
    ...
    match solver {
        SolverArg::ExactCover => {
            ...
            exact_cover_oracle::solve(component)
                .map_err(|error| CliError::Solver(error.to_string()))
        }
        SolverArg::SgExplicit => {
            ...
            sg_oracle::grid::solve(component).map_err(|error| CliError::Solver(error.to_string()))
        }
        SolverArg::DominanceC0 => {
            ...
            dominance::experiment::solve(component, Mode::ExplicitEdges)
                .map_err(|error| CliError::Solver(error.to_string()))
        }
...
const fn representation_kind(representation: RepresentationArg) -> Representation {
    match representation {
        RepresentationArg::Dominance4d => Representation::GeneralDominance4D,
        RepresentationArg::PathTree => Representation::CleanHoleFreePathTree,
        RepresentationArg::Auto => Representation::Auto,
    }
}
...
fn polygon_from_coordinates(
    outer: Vec<[i64; 2]>,
    holes: Vec<Vec<[i64; 2]>>,
) -> Result<RectilinearPolygon, mrd_domain::PolygonError> {
...
fn write_experiment_json(
    ...
    object.insert(
        "metadata".to_owned(),
        serde_json::to_value(verification::benchmark::BenchmarkMetadata {
...
fn benchmark_context() -> Result<verification::benchmark::BenchmarkContext, CliError> {
    ...
    let timestamp = SystemTime::now()
        ...
        .map_err(|error| CliError::Output(error.to_string()))?
        .as_secs();
    Ok(verification::benchmark::BenchmarkContext {
        ...
        rustc_version: String::from_utf8(rustc_output.stdout)
            ...
            .trim()
            .to_owned(),
        command: std::iter::once("mrd".to_owned())
...
fn update_manifest(
    path: &Path,
    metadata: verification::benchmark::BenchmarkMetadata,
) -> Result<(), CliError> {
...
fn write_svg_files(
    ...
    for (index, (component, solution)) in components.iter().zip(solutions).enumerate() {
        ...
        let svg = if solver == SolverArg::ExactCover {
            ...
            let analysis = sg_oracle::grid::analyze(component)
                .map_err(|error| CliError::Solver(error.to_string()))?;
            let (selected_horizontal, selected_vertical) = selected_chords(
                &solution.result,
...
fn persist_counterexample(error: &verification::grid::VerificationError) -> Result<(), CliError> {
    let Some(fixture) = error.fixture() else {
        return Ok(());
    };
    verification::minimize::write_regression_bundle(Path::new("test-data/regressions"), fixture)
        .map_err(|error| CliError::Output(error.to_string()))?;
    Ok(())
}
...
enum CliError {
    ...
    #[error(transparent)]
    Format(#[from] mrd_domain::FormatError),
...
mod tests {
    ...
    fn formal_polygon_json_auto_detection_builds_canonical_incidence() {
        ...
        let round_trip: mrd_domain::FormalRectilinearPolygon =
    ...
    fn native_polygon_fixture_corpus_validates_and_solves() {
        ...
        for name in [
            ...
            let formal = mrd_domain::FormalRectilinearPolygon::new(
                polygon.clone(),
                mrd_domain::Ornament::default(),
            )
            .unwrap();
            ...
            let result = dominance::experiment::solve_polygon(&polygon)
            ...
            if name == "scaled-complete-bipartite.json" {
                let families = sg_oracle::polygon::chord::oracle::Pairwise
                    .enumerate(&polygon)
                    .unwrap();
                ...
                assert!(families.horizontal.iter().all(|&horizontal| {
                    families
                        .vertical
                        .iter()
                        .all(|&vertical| mrd_domain::closed_chords_intersect(horizontal, vertical))
                }));
                let auto = dominance::experiment::solve_polygon_with_representation(
                    &polygon,
                    dominance::experiment::Representation::Auto,
                )
                .unwrap();
                assert_eq!(
