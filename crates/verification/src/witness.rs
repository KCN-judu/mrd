...
use dominance::experiment::{PathTreeOrientation, RegionBackend};
use dominance::path_tree::build_oriented_path_tree_partition_with_backend_and_options;
use mrd_domain::{ColorGrid, Diagnostics, DissectionResult, SvgOverlay, render_dissection_svg};
use serde::{Deserialize, Serialize};
use sg_oracle::grid::experiment::InteriorRuns;
use sg_oracle::grid::{analyze_geometry_with, classify_clean_hole_free_with_endpoint_index};
use thiserror::Error;
...
pub struct PathTreeWitness {
    ...
    pub boundary: mrd_domain::Boundary,
    pub dual_tree: dominance::path_tree::RegionDualTree,
    pub compact_paths: Vec<dominance::path_tree::CompactTreePath>,
    pub hld: dominance::path_tree::HeavyLightDecomposition,
    pub biclique_partition: dominance::biclique::Partition,
...
pub fn search_path_tree_witnesses(
    ...
    for candidate in candidates {
        ...
        {
            examined += 1;
            let Ok(geometry) = analyze_geometry_with(&component, &InteriorRuns) else {
                continue;
            };
            ...
            .find_map(|orientation| {
                build_oriented_path_tree_partition_with_backend_and_options(
                    ...
                    dominance::experiment::GapBackend::Experiment,
                )
                .ok()
                .and_then(|partition| {
            ...
            let Ok(canonical_geometry) = analyze_geometry_with(&canonical_component, &InteriorRuns)
            else {
                continue;
            };
...
fn witness_predicate(
    component: &mrd_domain::GridComponent<bool>,
    min_horizontal_chords: usize,
    min_vertical_chords: usize,
    min_dual_branching: usize,
    min_path_count: usize,
...
fn connected_component_from_cells(
    width: usize,
    height: usize,
    retained: &[mrd_domain::Cell],
) -> Option<mrd_domain::GridComponent<bool>> {
...
fn family_metrics(component: &mrd_domain::GridComponent<bool>) -> Option<FamilyMetrics> {
    let geometry = analyze_geometry_with(component, &InteriorRuns).ok()?;
    let certificate = classify_clean_hole_free_with_endpoint_index(
        component,
        &geometry.boundary,
        &geometry.horizontal_chords,
...
fn join_witness_components(
    first: &mrd_domain::GridComponent<bool>,
    second: &mrd_domain::GridComponent<bool>,
...
fn stored_witness_components() -> Vec<(mrd_domain::GridComponent<bool>, FamilyMetrics)> {
