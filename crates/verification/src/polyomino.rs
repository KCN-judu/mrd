...
fn solve_component(
    ...
    if component.cell_count() <= oracle_cell_limit {
        match exact_cover_oracle::solve(component) {
            Ok(result) => {
    ...
    for (name, result) in [
        (
            "sg-explicit",
            sg_oracle::grid::solve(component).map_err(|error| error.to_string()),
        ),
        (
            "dominance-c0",
        ...
        (
            ...
            dominance::experiment::solve(component, Mode::Compact)
                .map_err(|error| error.to_string()),
        ),
        (
    ...
    let hole_count = sg_oracle::grid::analyze(component)
        .ok()
