use mrd_domain::{DissectionResult, GridComponent, GridRect, ValidationError, validate_dissection};
...
fn transform_cell(
    ...
    height: usize,
    transform: GridTransform,
) -> Result<mrd_domain::Cell, TransformError> {
    let transformed = match transform {
        GridTransform::Translate { dx, dy } => mrd_domain::Cell {
            x: cell.x.checked_add(dx).ok_or(TransformError::Overflow)?,
            y: cell.y.checked_add(dy).ok_or(TransformError::Overflow)?,
        },
        GridTransform::ReflectHorizontal => mrd_domain::Cell {
            x: width - 1 - cell.x,
            y: cell.y,
        },
        GridTransform::ReflectVertical => mrd_domain::Cell {
            x: cell.x,
            y: height - 1 - cell.y,
        },
        GridTransform::Rotate90 => mrd_domain::Cell {
            x: height - 1 - cell.y,
            y: cell.x,
        },
        GridTransform::Rotate180 => mrd_domain::Cell {
            x: width - 1 - cell.x,
            y: height - 1 - cell.y,
        },
        GridTransform::Rotate270 => mrd_domain::Cell {
            x: cell.y,
            y: width - 1 - cell.x,
        },
        GridTransform::ReflectMainDiagonal => mrd_domain::Cell {
            x: cell.y,
            y: cell.x,
        },
        GridTransform::ReflectAntiDiagonal => mrd_domain::Cell {
            x: height - 1 - cell.y,
            y: width - 1 - cell.x,
        },
...
mod tests {
    ...
    fn every_transform_maps_every_solver_output_back_to_valid_geometry() {
        ...
        let original = grid
            ...
            .unwrap();
        let expected = exact_cover_oracle::solve(&original)
            .unwrap()
        ...
        for transform in transforms {
            ...
            let results = vec![
                exact_cover_oracle::solve(&transformed.component).unwrap(),
                sg_oracle::grid::solve(&transformed.component).unwrap(),
                dominance::experiment::solve(&transformed.component, Mode::ExplicitEdges).unwrap(),
                dominance::experiment::solve(&transformed.component, Mode::Compact).unwrap(),
                dominance::experiment::solve_with_verification_mode(
