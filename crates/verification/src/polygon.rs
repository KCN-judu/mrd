...
use dominance::experiment::{
    ...
    Representation, Verification, solve_polygon, solve_polygon_with_options,
};
use mrd_domain::{
...
use sg_oracle::grid::{CleanHoleFreeCertificate, EndpointIndex, classify_clean_polygon};
use sg_oracle::polygon::{
    self as sg_polygon, CoordinateCompressedCompletion, IndexedPolygonCompletion,
    PolygonChordEnumerationMetrics, PolygonCompletionResult, SweepCertificate,
...
pub struct PolygonGeometryEvidence {
    ...
    pub reflex_vertices: Vec<mrd_domain::Point>,
...
pub fn verify_polygon(
    ...
    let clean_path_tree = reference_geometry.clean_certificate.eligible.then(|| {
        solve_polygon_with_options(
            polygon,
            PolygonSolveOptions {
                representation: Representation::CleanHoleFreePathTree,
                ..audited_options
            },
        )
    });
...
fn geometry_evidence(
    ...
    families: &sg_oracle::grid::Families,
...
mod tests {
    ...
    use dominance::experiment::{
        ChordEnumerator, Representation, Verification, solve_polygon,
    ...
    use sg_oracle::grid::{
        HorizontalUnitCut, IndexedFrontierCompletion, VerticalUnitCut, analyze_geometry_with,
        complete_with_prepared_backend,
    };
    use sg_oracle::polygon::{
        CoordinateCompressedCompletion, HorizontalCutSegment, VerticalCutSegment,
    };
    ...
    fn compare_component(
        ...
        let geometry =
            analyze_geometry_with(component, &InteriorRuns).map_err(|error| error.to_string())?;
        ...
        let auto_result = solve_polygon_with_representation(&polygon, Representation::Auto)
            .map_err(|error| error.to_string())?;
    ...
    fn selected_grid_flags(
        result: &mrd_domain::DissectionResult,
        key: &str,
        len: usize,
    ) -> Vec<bool> {
