...
use dominance::{biclique::Partition, embedding::DominanceEmbedding};
...
fn collect_biclique_audits(fixture: &GridFixture) -> Result<Value, MinimizeError> {
    ...
    for component in grid.four_connected_components() {
        let value = (|| {
            let analysis =
                sg_oracle::grid::analyze(&component).map_err(|error| error.to_string())?;
            let embedding =
...
fn collect_solver_outputs(fixture: &GridFixture) -> Result<Value, MinimizeError> {
    ...
    for component in grid.four_connected_components() {
        ...
        solvers.insert(
            "exact-cover",
            result_value(exact_cover_oracle::solve(&component)),
        );
        solvers.insert(
            "sg-explicit",
            result_value(sg_oracle::grid::solve(&component)),
        );
        solvers.insert(
            "dominance-c0",
            result_value(dominance::experiment::solve(
                &component,
                Mode::ExplicitEdges,
            )),
        );
        solvers.insert(
            ...
            result_value(dominance::experiment::solve(&component, Mode::Compact)),
        );
        solvers.insert(
            "dominance-compact-only",
            result_value(dominance::experiment::solve_with_verification_mode(
                &component,
