...
impl AdversarialInstance {
    ...
    /// Writes the same JSON schema accepted by `mrd` plus fixture metadata.
    ///
    /// # Errors
    ///
...
fn asymmetric_path_witness() -> AdversarialInstance {
    let mask = 9_971_u16;
    let cells = (0..16)
        .map(|index| mask & (1_u16 << index) != 0)
        .collect::<Vec<_>>();
    instance(
        "asymmetric-path-witness",
        "path-tree-mixed-witness",
        4,
        4,
        cells,
        [("mask".to_owned(), usize::from(mask))]
            .into_iter()
            .collect(),
    )
}
...
/// whether a candidate is clean and what dual shape it realizes. The chain is
...
/// # Panics
///
/// Panics if the finite-grid constructor cannot represent the requested
/// positive chain scale. The public function clamps the scale to at least 3,
/// so ordinary calls within the supported grid range do not panic.
#[must_use]
pub fn path_tree_geometry_families(scale: usize) -> Vec<AdversarialInstance> {
    let scale = scale.max(3);
    let mut chain = clean_complete_bipartite_grid(scale)
        .expect("positive path-tree chain scale fits the finite-grid constructor");
    ...
    let mut asymmetric = asymmetric_path_witness();
...
#[cfg(test)]
mod tests {
    use dominance::embedding::{DominanceEmbedding, strict_dominance};
    use mrd_domain::closed_chords_intersect;
    use sg_oracle::grid::{
        Enumerator, IndexedFrontierCompletion, ReferenceRescanCompletion, analyze,
    ...
    use crate::grid::verify_component;
    ...
    fn clean_geometry_family_branching_probe() {
            ...
            .chain(path_tree_geometry_families(9))
        {
            for component in instance.foreground_components().unwrap() {
                ...
                let certificate = sg_oracle::grid::classify_clean_hole_free(
                    &component,
                    &geometry.boundary,
                ...
                for orientation in [
                    dominance::experiment::PathTreeOrientation::VerticalTreeHorizontalPaths,
                    dominance::experiment::PathTreeOrientation::HorizontalTreeVerticalPaths,
                ] {
                    let Ok(partition) =
                        dominance::path_tree::build_oriented_path_tree_partition_with_backend(
                            &geometry.prepared,
                            &geometry.boundary,
                            &geometry.horizontal_chords,
                            &geometry.vertical_chords,
                            ...
                            orientation,
                            false,
                            dominance::experiment::RegionBackend::Experiment,
                        )
                    else {
                        continue;
                    };
    ...
    #[test]
    fn geometry_families_cover_path_and_branching_witnesses() {
        let families = path_tree_geometry_families(7);
        let mut chain_path_incidence = 0;
        let mut star_branching = 0;
        let mut mixed_path_count = 0;
        for instance in families {
            let component = instance
                ...
                .unwrap();
            let geometry =
                sg_oracle::grid::analyze_geometry_with(&component, &InteriorRuns).unwrap();
            let certificate = sg_oracle::grid::classify_clean_hole_free(
                &component,
                &geometry.boundary,
                &geometry.horizontal_chords,
            ...
            assert!(certificate.eligible, "{} is not clean", instance.name);
            let mut fixed_partitions = Vec::new();
            for orientation in [
                dominance::experiment::PathTreeOrientationPolicy::VerticalTree,
                dominance::experiment::PathTreeOrientationPolicy::HorizontalTree,
            ] {
                let partition =
                    dominance::path_tree::build_path_tree_partition_with_orientation_policy(
                        &geometry.prepared,
                        &geometry.boundary,
                        &geometry.horizontal_chords,
                        &geometry.vertical_chords,
                    ...
                    .unwrap();
                fixed_partitions.push(partition);
            }
            if instance.family == "laminar-chain" {
                chain_path_incidence = fixed_partitions
                    .iter()
                    .map(|partition| partition.total_path_edge_incidences)
                    .max()
                    .unwrap_or(0);
            } else if instance.family == "laminar-star" {
                star_branching = fixed_partitions
                    .iter()
                    .map(|partition| {
                        partition
                            .path_tree
                    ...
                    .max()
                    .unwrap_or(0);
            } else if instance.family == "asymmetric-orientation" {
                mixed_path_count = fixed_partitions
                    .iter()
                    .map(|partition| partition.path_count)
                    .max()
                    .unwrap_or(0);
                assert!(!geometry.horizontal_chords.is_empty());
                assert!(!geometry.vertical_chords.is_empty());
            }
        }
        assert!(chain_path_incidence > 0);
        assert!(star_branching >= 3, "star branching={star_branching}");
        assert!(mixed_path_count > 0);
    }
    ...
    fn endpoint_component_pairs_are_checked_directly() {
        for instance in endpoint_contact_instances() {
            for component in instance.foreground_components().unwrap() {
                let analysis = sg_oracle::grid::analyze(&component).unwrap();
                let embedding =
    ...
    fn grid_runs_match_all_adversarial_chord_families() {
        ...
        for instance in instances {
            for component in instance.foreground_components().unwrap() {
                ...
                let reference = Pairwise.enumerate(&component, &boundary).unwrap();
                let grid_runs = InteriorRuns.enumerate(&component, &boundary).unwrap();
                assert_eq!(
    ...
    fn dense_generator_meets_requested_chord_floor() {
        for (horizontal, vertical) in [(4, 5), (8, 8), (16, 16), (32, 32)] {
            ...
            let c0 = dominance::experiment::solve(
                &component,
                dominance::experiment::Mode::ExplicitEdges,
            )
            .unwrap();
            let compact =
                dominance::experiment::solve(&component, dominance::experiment::Mode::Compact)
                    .unwrap();
            let audited_grid_runs =
                dominance::experiment::solve_with_verification_mode_and_chord_enumerator(
                    &component,
                    dominance::experiment::Verification::FullyAudited,
                    dominance::experiment::ChordEnumerator::GridInteriorRuns,
                )
                .unwrap();
            let compact_only = dominance::experiment::solve_with_verification_mode(
                &component,
                dominance::experiment::Verification::CompactOnly,
            )
            .unwrap();
            ...
            assert_eq!(
                ...
                mrd_domain::ExecutionTrace {
                    compact_structure_check_called: true,
                    ..mrd_domain::ExecutionTrace::default()
                }
            );
    ...
    fn clean_complete_bipartite_family_is_exact_and_clean() {
        for t in 1..=4 {
            ...
            let analysis = sg_oracle::grid::analyze_with(component, &InteriorRuns).unwrap();
            ...
            let path_tree = dominance::experiment::solve_with_representation(
                component,
                dominance::experiment::Verification::FullyAudited,
                dominance::experiment::Representation::CleanHoleFreePathTree,
                dominance::experiment::ChordEnumerator::GridInteriorRuns,
                sg_oracle::grid::CompletionBackendKind::ReferenceRescan,
            );
    ...
    fn clean_path_tree_compact_execution_remains_edge_free() {
        for t in 1..=3 {
            let instance = clean_complete_bipartite_grid(t).unwrap();
            let component = instance.foreground_components().unwrap().remove(0);
            let result = dominance::experiment::solve_with_representation(
                &component,
                dominance::experiment::Verification::CompactOnly,
                dominance::experiment::Representation::CleanHoleFreePathTree,
                dominance::experiment::ChordEnumerator::GridInteriorRuns,
                sg_oracle::grid::CompletionBackendKind::IndexedFrontier,
            )
            .unwrap();
