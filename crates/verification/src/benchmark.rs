...
use dominance::compressed_flow::experiment as flow_experiment;
use dominance::experiment::{
    ChordEnumerator, Representation, Verification, solve_with_representation_and_region_dual,
...
use graph::{DinicBackend, PushRelabelBackend};
use mrd_domain::{ColorGrid, Diagnostics, ExactRatio, GridComponent};
...
use crate::grid::{GridFixture, VerificationError, verify_component};
...
pub fn clean_boundary_differential_4x4(
    ...
    for mask in 1_u32..(1_u32 << 16) {
        ...
        {
            components += 1;
            let Ok(geometry) = sg_oracle::grid::analyze_geometry_with(
                &component,
...
fn append_orientation_audit_component<C>(
    ...
    population: &str,
    instance_name: &str,
    component: &mrd_domain::GridComponent<C>,
) where
    C: Clone + Eq,
{
    let Ok(geometry) = sg_oracle::grid::analyze_geometry_with(
    ...
    let exact = dominance::path_tree::build_path_tree_partition_with_orientation_policy(
        &geometry.prepared,
        &geometry.boundary,
        &geometry.horizontal_chords,
        &geometry.vertical_chords,
        ...
        false,
        dominance::experiment::RegionBackend::Experiment,
        dominance::experiment::PathTreeOrientationPolicy::BuildBothExact,
    );
    let build_both_construction_microseconds = build_both_started.elapsed().as_micros();
    let bound_started = Instant::now();
    let estimated = dominance::path_tree::build_path_tree_partition_with_orientation_policy(
        &geometry.prepared,
        &geometry.boundary,
        &geometry.horizontal_chords,
        &geometry.vertical_chords,
        ...
        false,
        dominance::experiment::RegionBackend::Experiment,
        dominance::experiment::PathTreeOrientationPolicy::BoundEstimate,
    );
    ...
    for (population, instance) in collect_dual_differential_instances(sizes) {
        ...
        for component in components {
            ...
            let area = solve_with_representation_and_region_dual_and_orientation_policy(
                ...
                ChordEnumerator::GridInteriorRuns,
                CompletionBackendKind::IndexedFrontier,
                dominance::experiment::RegionBackend::Oracle,
                dominance::experiment::PathTreeOrientationPolicy::BuildBothExact,
            );
...
fn clean_rejection_name(reason: &sg_oracle::grid::CleanRejectionReason) -> String {
    match reason {
        sg_oracle::grid::CleanRejectionReason::MultipleOuterLoops { .. } => {
            "multiple-outer-loops".to_owned()
        }
        sg_oracle::grid::CleanRejectionReason::HasHole { .. } => "has-hole".to_owned(),
        sg_oracle::grid::CleanRejectionReason::UnsupportedOrnamentModel => {
            "unsupported-ornament-model".to_owned()
        }
        sg_oracle::grid::CleanRejectionReason::NonProperHorizontalChord(_) => {
            "non-proper-horizontal-chord".to_owned()
        }
        sg_oracle::grid::CleanRejectionReason::NonProperVerticalChord(_) => {
            "non-proper-vertical-chord".to_owned()
        }
        sg_oracle::grid::CleanRejectionReason::EndpointNotOnBoundary => {
            "endpoint-not-on-boundary".to_owned()
        }
        sg_oracle::grid::CleanRejectionReason::SharedBoundaryEndpoint { .. } => {
...
pub fn benchmark_biclique_construction(
    ...
    let command = context.command.split_once(" benchmark").map_or_else(
        ...
        |(_, suffix)| format!("mrd benchmark{suffix}"),
    );
    let instances = sizes
        .iter()
...
fn benchmark_biclique_component<C>(
    ...
    let geometry = match sg_oracle::grid::analyze_geometry(component) {
...
pub fn benchmark_dense_compact_only(context: BenchmarkContext, sizes: &[usize]) -> BenchmarkReport {
    ...
    for instance in &instances {
        match instance.foreground_components() {
            Ok(components) => {
                for component in components {
                    let result = dominance::experiment::solve_with_verification_mode(
                        &component,
                        Verification::CompactOnly,
                    );
                    rows.push(match result {
...
pub fn benchmark_dense_completion(context: BenchmarkContext, sizes: &[usize]) -> BenchmarkReport {
    ...
    for (backend, backend_name) in backends {
        for instance in &instances {
            if let Ok(components) = instance.foreground_components() {
                for component in components {
                    let result = dominance::experiment::solve_with_verification_mode_and_chord_enumerator_and_completion_backend(
                        &component,
                        Verification::CompactOnly,
...
pub fn benchmark_clean_complete_bipartite_compact(
    ...
    for &size in sizes {
        ...
        for component in components {
            let path = dominance::experiment::solve_with_representation(
                &component,
                Verification::CompactOnly,
            ...
            let general = dominance::experiment::solve_with_representation(
                &component,
                Verification::CompactOnly,
                dominance::experiment::Representation::GeneralDominance4D,
                dominance::experiment::ChordEnumerator::GridInteriorRuns,
                CompletionBackendKind::IndexedFrontier,
            );
...
pub fn benchmark_path_tree_geometry_families(
    ...
    for instance in instances {
        ...
        for component in components {
            let geometry = match sg_oracle::grid::analyze_geometry_with(
                &component,
                &sg_oracle::grid::experiment::InteriorRuns,
            ) {
                Ok(geometry) => geometry,
...
pub fn benchmark_path_tree_vs_4d(context: BenchmarkContext, sizes: &[usize]) -> PathTreeVs4dReport {
    ...
    for instance in instances {
        ...
        for component in components {
            ...
            let phase = |result: &mrd_domain::DissectionResult, key: &str| {
                result.diagnostics.phase_microseconds.get(key).copied()
            };
            let total = |result: &mrd_domain::DissectionResult| {
...
fn canonical_component_key<C>(component: &mrd_domain::GridComponent<C>) -> String
where
    C: Clone + Eq,
{
...
pub fn benchmark_path_tree_comparison(
    ...
    for mask in 1_u32..(1_u32 << bit_count) {
        ...
        for component in grid
            ...
            .filter(|component| component.color)
        {
            let Ok(geometry) = sg_oracle::grid::analyze_geometry(&component) else {
                continue;
            };
            let certificate = sg_oracle::grid::classify_clean_hole_free(
                &component,
