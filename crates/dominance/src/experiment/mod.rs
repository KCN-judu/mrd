...
use crate::{compressed_flow, path_tree};
use graph::{DinicBackend, FlowBackendKind, MaxFlowBackend, hopcroft_karp};
use mrd_domain::{
...
use serde_json::json;
use sg_oracle::grid::{
    CompletionBackendKind, CompletionMetrics, CoordinateCompressedCompletion, EndpointIndex,
    Enumerator, IndexedFrontierCompletion, IndexedPolygonCompletion, PolygonSgError,
    ReferenceRescanCompletion, SgError, analyze_prepared_geometry, audit_sweep_provenance,
    classify_clean_polygon, complete_with_prepared_backend, validate_polygon_dissection_count,
};
use sg_oracle::grid::{experiment::InteriorRuns, oracle::Pairwise};
use sg_oracle::{polygon as sg_polygon, polygon_arrangement, polygon_cut_index, polygon_sparse};
use thiserror::Error;
...
pub struct PolygonSolveOptions {
    pub verification_mode: Verification,
...
impl Default for PolygonSolveOptions {
    fn default() -> Self {
        Self {
            ...
            representation: Representation::GeneralDominance4D,
        }
    }
}
...
/// # Errors
///
/// Returns [`enum@Error`] when polygon validation, chord enumeration,
/// compact matching, completion, or final validation fails.
pub fn solve_polygon(polygon: &RectilinearPolygon) -> Result<PolygonDissectionResult, Error> {
    solve_polygon_with_representation(polygon, Representation::GeneralDominance4D)
}
...
/// # Errors
///
/// Returns [`enum@Error`] for backend disagreement or any solver failure.
#[allow(clippy::too_many_lines)]
pub fn solve_polygon_with_options(
    ...
    let (families, chord_metrics, sweep_certificate) = match options.verification_mode {
        Verification::FullyAudited => {
            ...
            {
                return Err(Error::ChordFamilyMismatch);
            }
        ...
        Verification::CompactOnly => match options.chord_backend {
            PolygonChordBackend::ReferencePairwise => {
    ...
    let endpoint_index =
        EndpointIndex::new(boundary_index, &families.horizontal, &families.vertical)?;
    ...
    let four_d_construction = match options.verification_mode {
        Verification::FullyAudited => Partition::comparability_theorem_8_audited(&embedding)?,
        Verification::CompactOnly => crate::biclique::experiment::construct(&embedding)?,
    };
    ...
    let (selected_partition, selected_flow, representation_name) = match options.representation {
        Representation::CleanHoleFreePathTree | Representation::Auto
            if clean_certificate.eligible =>
        {
            ...
            if flow.flow.value != four_d_flow.flow.value {
                return Err(Error::PathTreeMatchingMismatch {
            ...
            (
                partition,
                flow,
                Representation::CleanHoleFreePathTree.name(),
            )
        }
        Representation::CleanHoleFreePathTree => {
            return Err(Error::PathTreeIneligible(clean_certificate));
        }
        Representation::GeneralDominance4D | Representation::Auto => (
            four_d_partition.clone(),
            four_d_flow.clone(),
            Representation::GeneralDominance4D.name(),
        ),
    };
    let flow_at = Instant::now();
    let flow_value =
        usize::try_from(selected_flow.flow.value).map_err(|_| Error::FlowValueConversion)?;
    ...
    let completion = match options.verification_mode {
        Verification::FullyAudited => {
            ...
            {
                return Err(Error::PolygonCompletionMismatch);
            }
        ...
        Verification::CompactOnly => match options.completion_backend {
    ...
    if completion.rectangles.len() != optimum_rectangle_count {
        return Err(Error::CompletionCount {
            expected: optimum_rectangle_count,
            actual: completion.rectangles.len(),
        });
    }
    if options.verification_mode == Verification::FullyAudited {
    ...
    let dense_arrangement_used = options.verification_mode == Verification::FullyAudited
...
/// # Errors
///
/// Returns [`enum@Error`] when eligibility, representation, flow,
...
pub fn solve_with_representation<C>(
    ...
    solve_with_representation_and_region_dual(
        ...
        completion_backend,
        match mode {
            Verification::FullyAudited => RegionBackend::Oracle,
            Verification::CompactOnly => RegionBackend::Experiment,
        },
    )
}
...
/// # Errors
///
/// Returns [`enum@Error`] when geometry, representation, flow,
/// completion, or validation invariants fail.
...
pub fn solve_with_representation_and_path_tree_options<C>(
    ...
    match representation {
        Representation::GeneralDominance4D => {
        ...
        Representation::CleanHoleFreePathTree => solve_path_tree_dispatch(
            component,
            mode,
            enumerator,
            completion_backend,
        ...
        Representation::Auto => {
            let geometry = match enumerator {
                ChordEnumerator::ReferencePairwise => {
                    sg_oracle::grid::analyze_geometry_with(component, &Pairwise)?
                }
                ChordEnumerator::GridInteriorRuns => {
                    sg_oracle::grid::analyze_geometry_with(component, &InteriorRuns)?
                }
            };
            let certificate = sg_oracle::grid::classify_clean_hole_free_with_endpoint_index(
                component,
                &geometry.boundary,
...
const fn default_orientation_policy(mode: Verification) -> PathTreeOrientationPolicy {
    match mode {
        // BoundEstimate remains an explicit benchmark policy; the expanded
        // v0.8 witness population contains positive-regret cases.
        Verification::FullyAudited | Verification::CompactOnly => {
...
fn annotate_general_representation(
    ...
    result.diagnostics.conflict_representation =
        Some(Representation::GeneralDominance4D.name().to_owned());
...
/// # Errors
///
/// Returns [`enum@Error`] when a construction, flow, completion, or output
/// invariant fails.
...
/// # Errors
///
/// Returns [`enum@Error`] when geometry, dominance, flow, completion, or
...
/// # Errors
///
/// Returns [`enum@Error`] when geometry, flow, completion, or validation
/// invariants fail.
...
/// # Errors
///
/// Returns [`enum@Error`] when any geometric equivalence, biclique,
/// matching/flow, cover, completion, or output invariant fails.
#[allow(clippy::too_many_lines)]
pub fn solve<C>(component: &GridComponent<C>, mode: Mode) -> Result<DissectionResult, Error> {
...
#[allow(clippy::too_many_lines)]
fn solve_fully_audited_with<C, E: Enumerator>(
    component: &GridComponent<C>,
    mode: Mode,
    enumerator: &E,
    completion_backend: CompletionBackendKind,
...
#[allow(clippy::too_many_lines)]
fn solve_fully_audited_with_backend<C, E: Enumerator, B: MaxFlowBackend>(
    component: &GridComponent<C>,
    mode: Mode,
    enumerator: &E,
    completion_backend: CompletionBackendKind,
    ...
    if enumerator.name() == "grid-interior-runs" {
        let reference = sg_oracle::grid::analyze_geometry(component)?;
    ...
    if dominance_edges != sg_edges {
        return Err(Error::ExplicitGraphMismatch);
    }
    let embedding_at = Instant::now();

    let partition = match mode {
        Mode::ExplicitEdges => Partition::from_explicit_edges(&dominance_graph),
        Mode::Compact => Partition::comparability_theorem_8_audited(&embedding)?.partition,
    };
    ...
    let flow_value =
        usize::try_from(flow_solution.flow.value).map_err(|_| Error::FlowValueConversion)?;
    if flow_value != sg_analysis.matching.size {
        return Err(Error::MatchingFlowMismatch {
    ...
    for (left, right) in dominance_graph.edges() {
        if !flow_solution.vertex_cover.left[left] && !flow_solution.vertex_cover.right[right] {
            return Err(Error::UncoveredConflictEdge { left, right });
        }
    }
    ...
    let result = DissectionResult {
        ...
        diagnostics: Diagnostics {
            ...
            execution_trace: ExecutionTrace {
                ...
                c0_partition_built: matches!(mode, Mode::ExplicitEdges),
        ...
        certificate: Some(Certificate {
            kind: match mode {
                Mode::ExplicitEdges => "dominance-c0",
                Mode::Compact => "dominance-compact",
            }
            .to_owned(),
            payload: json!({
...
#[allow(clippy::too_many_lines)]
fn solve_compact_only_with<C, E: Enumerator>(
    component: &GridComponent<C>,
    enumerator: &E,
    completion_backend: CompletionBackendKind,
    embedding_backend: EmbeddingCoordinateBackend,
    ...
    owned_allocation_estimates.insert(
        ...
        total_chord_count * size_of::<mrd_domain::HorizontalChord>(),
    );
    owned_allocation_estimates.insert(
    ...
    owned_allocation_estimates.insert(
        ...
        total_chord_count * size_of::<crate::embedding::DominancePoint>(),
    );
    owned_allocation_estimates.insert(
        "biclique_vectors".to_owned(),
        partition.blocks.len() * size_of::<crate::biclique::Block>()
    ...
    owned_allocation_estimates.insert(
        ...
        completion.selected_horizontal_unit_cuts.len()
            * size_of::<sg_oracle::grid::HorizontalUnitCut>()
            + completion.selected_vertical_unit_cuts.len()
                * size_of::<sg_oracle::grid::VerticalUnitCut>()
            + completion.added_horizontal_unit_cuts.len()
                * size_of::<sg_oracle::grid::HorizontalUnitCut>()
            + completion.added_vertical_unit_cuts.len()
                * size_of::<sg_oracle::grid::VerticalUnitCut>()
            + completion.rectangles.len() * size_of::<mrd_domain::GridRect>(),
    );
    owned_allocation_estimates.insert(
...
fn solve_path_tree_dispatch<C>(
    ...
    if !certificate.eligible {
        return Err(Error::PathTreeIneligible(certificate));
    }
    solve_path_tree_with_geometry(
        component,
        &geometry,
...
fn solve_path_tree_with_geometry<C>(
    ...
    certificate: &sg_oracle::grid::CleanHoleFreeCertificate,
    ...
    let path_tree = build_path_tree_partition_with_orientation_policy_and_options(
        ...
        &geometry.vertical_chords,
        certificate.clone(),
        mode == Verification::FullyAudited,
        region_dual,
        orientation_policy,
    ...
    if mode == Verification::FullyAudited {
        ...
        for &horizontal in &geometry.horizontal_chords {
            let horizontal_endpoints =
                sg_oracle::grid::horizontal_chord_endpoints(&geometry.boundary, horizontal)?;
            for &vertical in &geometry.vertical_chords {
                let vertical_endpoints =
                    sg_oracle::grid::vertical_chord_endpoints(&geometry.boundary, vertical)?;
                let loop_len = geometry
                    .boundary
                    .loop_len(horizontal_endpoints.first.loop_id)
                    .ok_or(Error::PathTreeAlternationMismatch)?;
                if mrd_domain::closed_chords_intersect(horizontal, vertical)
                    != sg_oracle::grid::endpoints_alternate(
                        horizontal_endpoints,
                        vertical_endpoints,
                        loop_len,
                    )
                {
                    return Err(Error::PathTreeAlternationMismatch);
                }
            }
        }
    }
    ...
    owned_allocation_estimates.insert(
        ...
        path_tree
            ...
            .edges
            .len()
            .saturating_mul(size_of::<(usize, usize, mrd_domain::VerticalChordId)>() * 2)
    ...
    owned_allocation_estimates.insert(
        ...
        path_tree.path_tree.tree.edges.len() * size_of::<path_tree::DualTreeEdge>()
            + path_tree.path_tree.tree.adjacency.len()
                * size_of::<Vec<(path_tree::DualRegionId, mrd_domain::VerticalChordId)>>()
            + path_tree
                .path_tree
                .tree
                .adjacency
                ...
                .map(Vec::len)
                .sum::<usize>()
                * size_of::<(path_tree::DualRegionId, mrd_domain::VerticalChordId)>(),
    );
    owned_allocation_estimates.insert(
    ...
    owned_allocation_estimates.insert(
        ...
        path_tree
            ...
            .chain_edges
            .iter()
            .map(|edges| edges.len() * size_of::<mrd_domain::VerticalChordId>())
            .sum(),
    );
    owned_allocation_estimates.insert(
        "canonical_segment_nodes".to_owned(),
        path_tree.canonical_segment_node_count * size_of::<crate::biclique::Block>(),
    );
    owned_allocation_estimates.insert(
        "biclique_vectors".to_owned(),
        path_tree.biclique_partition.blocks.len() * size_of::<crate::biclique::Block>()
            + sigma * size_of::<usize>(),
    );
    owned_allocation_estimates.insert(
    ...
    let result = DissectionResult {
        ...
        diagnostics: Diagnostics {
            ...
            .collect(),
            execution_trace: ExecutionTrace {
                pairwise_embedding_audit_called: mode == Verification::FullyAudited,
                explicit_conflict_graph_built: mode == Verification::FullyAudited,
                hopcroft_karp_called: mode == Verification::FullyAudited,
                full_edge_partition_audit_called: mode == Verification::FullyAudited,
                compact_structure_check_called: true,
                full_tree_path_edge_lists_materialized: mode == Verification::FullyAudited,
                per_path_bfs_called: mode == Verification::FullyAudited,
                area_flood_fill_dual_built: region_dual == RegionBackend::Oracle,
                unit_chord_cuts_materialized: region_dual == RegionBackend::Oracle,
                prepared_occupancy_transposed: mode == Verification::FullyAudited
            ...
            conflict_representation: Some(Representation::CleanHoleFreePathTree.name().to_owned()),
            ...
            dual_allocated_bytes: Some(
                path_tree.path_tree.tree.edges.len()
                    * std::mem::size_of::<mrd_domain::VerticalChordId>()
                    + path_tree.path_tree.tree.adjacency.len()
                        * std::mem::size_of::<
                            Vec<(path_tree::DualRegionId, mrd_domain::VerticalChordId)>,
                        >(),
            ),
...
fn complete_selected<C>(
    ...
    horizontal_chords: &[mrd_domain::HorizontalChord],
    vertical_chords: &[mrd_domain::VerticalChord],
    selected_horizontal: &[bool],
    selected_vertical: &[bool],
    backend: CompletionBackendKind,
) -> Result<sg_oracle::grid::CompletionResult, Error> {
    match backend {
...
pub enum Error {
    ...
    PathTreeIneligible(sg_oracle::grid::CleanHoleFreeCertificate),
...
mod polygon_tests {
    ...
    fn solves_native_large_gap_and_auto_path_tree_polygon() {
        ...
        let auto = solve_polygon_with_representation(&polygon, Representation::Auto).unwrap();
    ...
    fn polygon_with_hole_falls_back_to_four_dimensions() {
        ...
        .unwrap();
        let result = solve_polygon_with_representation(&polygon, Representation::Auto).unwrap();
    ...
    fn selected_flow_backends_agree_on_a_grid_dissection() {
        ...
        let dinic =
            solve_with_flow_backend(&component, Mode::Compact, FlowBackendKind::Dinic).unwrap();
        let push_relabel =
            solve_with_flow_backend(&component, Mode::Compact, FlowBackendKind::PushRelabel)
                .unwrap();
        assert_eq!(
    ...
    fn verify_grid_polygon_masks(width: usize, height: usize) -> usize {
        ...
        for mask in 1_u32..mask_limit {
                ...
                .filter(|component| component.color)
            {
                ...
                let geometry = analyze_geometry_with(&component, &InteriorRuns).unwrap();
                let polygon_families = sg_oracle::polygon::chord::oracle::Pairwise
                    .enumerate(&polygon)
                    .unwrap();
                let polygon_prepared = PreparedPolygonContext::new(&polygon).unwrap();
                let indexed_families = sg_oracle::polygon::chord::oracle::Indexed
                    .enumerate_prepared(&polygon_prepared)
                    .unwrap();
                let sweep_families = sg_oracle::polygon::chord::experiment::Sweep
                    .enumerate_prepared(&polygon_prepared)
                    .unwrap();
    ...
    fn selected_flags(result: &mrd_domain::DissectionResult, key: &str, len: usize) -> Vec<bool> {
    ...
    fn polygon_selected_flags(
        result: &mrd_domain::PolygonDissectionResult,
        key: &str,
        len: usize,
    ) -> Vec<bool> {
