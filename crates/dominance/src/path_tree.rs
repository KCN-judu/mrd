...
//!
//! `oracle::build_region_dual` is the deliberately redundant audited Oracle: it
//! labels the dual by cutting unit cell sides and flood-filling local
//! occupancy. `RegionBackend::Experiment` is the compact laminar backend: it
...
use sg_oracle::grid::{CleanHoleFreeCertificate, EndpointIndex};
use thiserror::Error;
...
pub enum RegionBackend {
    #[serde(rename = "reference-area-flood-fill")]
    Oracle,
    #[serde(rename = "boundary-laminar")]
    Experiment,
}
...
#[serde(rename_all = "kebab-case")]
pub enum GapBackend {
    #[serde(rename = "reference-nested")]
    Oracle,
    #[serde(rename = "event-sweep")]
    Experiment,
}
...
impl GapBackend {
    ...
    pub const fn name(self) -> &'static str {
        match self {
            Self::Oracle => "reference-nested",
            Self::Experiment => "event-sweep",
        }
    }
}
...
impl RegionBackend {
    ...
    pub const fn name(self) -> &'static str {
        match self {
            Self::Oracle => "reference-area",
            Self::Experiment => "boundary-laminar",
        }
    }
}
...
pub(super) struct BoundaryInterval {
    ...
    pub(super) end: usize,
    pub(super) chord: VerticalChordId,
}
...
pub fn build_boundary_laminar_dual_tree_with_options(
    ...
    vertical_chords: &[VerticalChord],
    certificate: &CleanHoleFreeCertificate,
    endpoint_index: Option<&EndpointIndex>,
    ...
    for &chord in vertical_chords {
        let endpoints = if let Some(endpoint_index) = endpoint_index {
            ...
            sg_oracle::grid::vertical_chord_endpoints(boundary, chord)
...
fn label_boundary_gaps(
    ...
    match backend {
        GapBackend::Oracle => Ok(oracle::label_gaps(boundary_len, intervals, depths)),
        GapBackend::Experiment => experiment::label_gaps(boundary_len, intervals),
    }
}

#[derive(Clone, Debug, Eq, PartialEq)]
pub(super) struct GapLabelResult {
    pub(super) labels: Vec<DualRegionId>,
    pub(super) membership_tests: usize,
    pub(super) event_push_count: usize,
    pub(super) event_pop_count: usize,
}
...
fn build_horizontal_boundary_laminar_dual_tree_with_options(
    ...
    for &chord in horizontal_chords {
        let endpoints = if let Some(endpoint_index) = endpoint_index {
            ...
            sg_oracle::grid::horizontal_chord_endpoints(boundary, chord)
                .map_err(|_| PathTreeError::InvalidBoundaryDual)
        }?;
        if endpoints.first.loop_id.0 != 0
            || endpoints.second.loop_id.0 != 0
...
impl RegionDualTree {
    fn region_at_horizontal_endpoint_boundary(
        ...
        let point =
            mrd_domain::Point::new(if first { chord.left() } else { chord.right() }, chord.y());
    ...
    fn region_at_horizontal_endpoint_boundary_id(
        &self,
        boundary: &Boundary,
        vertex_id: mrd_domain::BoundaryVertexId,
    ) -> Result<DualRegionId, PathTreeError> {
    ...
    fn region_at_vertical_endpoint_boundary(
        ...
        let point =
            mrd_domain::Point::new(chord.x(), if first { chord.bottom() } else { chord.top() });
        let vertex_id = boundary
            .vertex_id(point)
            .ok_or(PathTreeError::MissingPathEndpoint)?;
...
pub fn build_path_tree_partition_with_backend_and_options(
    ...
    let tree = match dual_backend {
        RegionBackend::Oracle => {
            oracle::build_region_dual(prepared, vertical_chords, &certificate)?
        }
        RegionBackend::Experiment => build_boundary_laminar_dual_tree_with_options(
            boundary,
            vertical_chords,
            &certificate,
            endpoint_index,
    ...
    let compact_paths = match dual_backend {
        RegionBackend::Oracle => tree.horizontal_endpoint_paths(prepared, horizontal_chords)?,
    ...
    let paths = if materialize_explicit_paths {
        if dual_backend == RegionBackend::Experiment {
    ...
    for ((chain, begin, end), mut left) in grouped {
        ...
        if !left.is_empty() && !right.is_empty() {
            bicliques.push(Block {
                id: mrd_domain::BicliqueId(bicliques.len()),
                left,
                right,
            });
        }
...
fn transpose_boundary(boundary: &Boundary) -> Boundary {
    ...
    let reflex_vertices = boundary
        .reflex_vertices
        .iter()
        .map(|vertex| mrd_domain::ReflexVertex {
            point: mrd_domain::Point::new(vertex.point.y, vertex.point.x),
        })
        .collect();
    let unit_edges = boundary
        ...
        .map(|(first, second)| {
            (
                mrd_domain::Point::new(first.y, first.x),
                mrd_domain::Point::new(second.y, second.x),
            )
        })
        .collect();
    Boundary {
...
pub fn build_oriented_path_tree_partition_with_backend_and_options(
    ...
    match orientation {
        ...
        PathTreeOrientation::HorizontalTreeVerticalPaths => {
            if dual_backend == RegionBackend::Experiment && !materialize_explicit_paths {
            ...
            let transposed = if dual_backend == RegionBackend::Experiment {
...
mod tests {
    ...
    use mrd_domain::{ColorGrid, validate_dissection};
    use sg_oracle::grid::{analyze_geometry, classify_clean_hole_free};

    use super::{
        DualRegionId, DualTreeEdge, GapBackend, HeavyLightDecomposition, PathTreeOrientation,
        PathTreeOrientationPolicy, RegionBackend, RegionDualTree, VerticalChordId,
        build_boundary_laminar_dual_tree, build_boundary_laminar_dual_tree_with_options,
    ...
    fn event_sweep_pops_equal_end_intervals_inner_first() {
        ...
        let depths = vec![1, 2];
        let reference =
            super::label_boundary_gaps(5, &intervals, &depths, GapBackend::Oracle).unwrap();
        let event =
            super::label_boundary_gaps(5, &intervals, &depths, GapBackend::Experiment).unwrap();
    ...
    fn path_tree_solver_matches_general_four_dimensional_solver() {
        ...
        for mask in 1_u16..(1_u16 << 9) {
            ...
            for component in ColorGrid::new(3, 3, cells)
                ...
                .filter(|component| component.color)
            {
                let Ok(path_tree) = crate::experiment::solve_with_representation(
                    &component,
                    crate::experiment::Verification::FullyAudited,
                    crate::experiment::Representation::CleanHoleFreePathTree,
                    crate::experiment::ChordEnumerator::GridInteriorRuns,
                    sg_oracle::grid::CompletionBackendKind::ReferenceRescan,
                ) else {
                    continue;
                };
                let general = crate::experiment::solve_with_verification_mode_and_chord_enumerator_and_completion_backend(
                    &component,
                    crate::experiment::Verification::FullyAudited,
                    crate::experiment::ChordEnumerator::GridInteriorRuns,
                    sg_oracle::grid::CompletionBackendKind::ReferenceRescan,
    ...
    fn compact_boundary_dual_matches_general_on_small_clean_population() {
        ...
        for mask in 1_u16..(1_u16 << 9) {
                ...
                .filter(|component| component.color)
            {
                let Ok(path_tree) = crate::experiment::solve_with_representation(
                    &component,
                    crate::experiment::Verification::CompactOnly,
                    crate::experiment::Representation::CleanHoleFreePathTree,
                    crate::experiment::ChordEnumerator::GridInteriorRuns,
                    sg_oracle::grid::CompletionBackendKind::ReferenceRescan,
                ) else {
    ...
    fn auto_representation_falls_back_for_holes() {
        ...
        let component = ColorGrid::new(3, 3, cells)
            ...
            .unwrap();
        let result = crate::experiment::solve_with_representation(
            &component,
            crate::experiment::Verification::CompactOnly,
            crate::experiment::Representation::Auto,
            crate::experiment::ChordEnumerator::GridInteriorRuns,
            sg_oracle::grid::CompletionBackendKind::ReferenceRescan,
        )
        .unwrap();
        ...
        assert!(matches!(
            crate::experiment::solve_with_representation(
                &component,
                crate::experiment::Verification::CompactOnly,
                crate::experiment::Representation::CleanHoleFreePathTree,
                crate::experiment::ChordEnumerator::GridInteriorRuns,
            ...
            Err(crate::experiment::Error::PathTreeIneligible(_))
        ));
    }
}
