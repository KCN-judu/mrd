...
pub fn verify_component<C>(
    ...
    let exact_cover = (component.cell_count() <= exact_cover_cell_limit)
        .then(|| rect_oracle_exact_cover::solve(component))
        .transpose()
    ...
    let sg_explicit =
        rect_oracle_sg::solve(component).map_err(|error| VerificationError::Solver {
            solver: "sg-explicit",
            message: error.to_string(),
        })?;
    let dominance_c0 =
        rect_dominance::solve(component, DominanceMode::ExplicitEdges).map_err(|error| {
    ...
    let dominance_compact =
        rect_dominance::solve(component, DominanceMode::Compact).map_err(|error| {
            VerificationError::Solver {
