...
use crate::grid::Families;
use crate::grid::{ChordRef, CleanHoleFreeCertificate, CleanRejectionReason, EndpointIndex};
...
fn prepare_formal_completion(
    ...
    for segment in &incidence.elementary_segments {
        if !segment
            .sources
            .iter()
            .any(|source| matches!(source, mrd_domain::FormalBoundarySource::Ornament { .. }))
        {
            continue;
        }
...
pub fn audit_sweep_provenance(
    ...
    for record in &certificate.output_records {
        ...
        let direction = match (record.axis, increasing) {
            (SweepAxis::Horizontal, true) => mrd_domain::OrthogonalDirection::East,
            (SweepAxis::Horizontal, false) => mrd_domain::OrthogonalDirection::West,
            (SweepAxis::Vertical, true) => mrd_domain::OrthogonalDirection::North,
            (SweepAxis::Vertical, false) => mrd_domain::OrthogonalDirection::South,
        };
        if prepared
            .edge_index()
...
fn reference_chord_keys(families: &Families) -> BTreeSet<(SweepAxis, i64, i64, i64)> {
    families
        .horizontal
        .iter()
        .map(|chord| {
...
fn sweep_family_keys(families: &Families) -> BTreeSet<(SweepAxis, i64, i64, i64)> {
...
impl<'a> IndexedPolygonCompletionState<'a> {
    ...
    fn ray_stop(
        ...
        let boundary = self.prepared.edge_index().nearest_boundary_blocker(
            point,
            match direction {
                PolygonDirection::East => mrd_domain::OrthogonalDirection::East,
                PolygonDirection::North => mrd_domain::OrthogonalDirection::North,
                PolygonDirection::West => mrd_domain::OrthogonalDirection::West,
                PolygonDirection::South => mrd_domain::OrthogonalDirection::South,
            },
        );
...
mod tests {
    ...
    use crate::grid::{Enumerator, experiment};
    use crate::polygon_arrangement;
    ...
    fn formal_empty_ornament_matches_ordinary_local_measure_and_pairwise_chords() {
        ...
        for polygon in fixtures {
            ...
            let formal_chords =
                mrd_domain::formal_polygon::oracle::effective_chords(&formal).unwrap();
            assert_eq!(formal_chords.horizontal, ordinary.horizontal);
            assert_eq!(formal_chords.vertical, ordinary.vertical);
            let source = mrd_domain::formal_polygon::experiment::effective_chords(&formal).unwrap();
    ...
    fn grid_derived_polygon_chords_match_on_all_3x3_masks() {
        ...
        for mask in 1_u16..1 << 9 {
                ...
                .filter(|component| component.color)
            {
                ...
                let grid_families = experiment::InteriorRuns
